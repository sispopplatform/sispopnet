#!/usr/bin/env python3
#
# python wsgi application for managing many sispopnet instances
#

__doc__ = """sispopnet bootserv wsgi app
also handles webhooks for CI
run me with via gunicorn pysispopnet.bootserv:app
"""
...
def handle_serve_sispopnet(modified_since, respond):
    l = BinHolder('sispopnet.zip')
    return l.serve(modified_since, respond)


def fetch_sispopnet(j, ref="staging", name="build:linux"):
    holder = BinHolder("sispopnet.zip")
    if 'builds' not in j:
        return False
    selected = None
    attrs = dict()
    ...
    if selected and 'id' in selected:
        url = 'https://gitlab.com/sispopproject/sispop-network/-/jobs/{}/artifacts/download'.format(selected['id'])
        r = requests.get(url)
...
def app(environ, start_response):
    ...
    if method.upper() == "PUT" and request_body_size > 0:
        ...
        elif environ.get("PATH_INFO") == "/sispopnet.zip":
            return handle_serve_sispopnet(environ.get("HTTP_IF_MODIFIED_SINCE"),start_response)
        elif environ.get("PATH_INFO") == "/":
            return response("200 OK", "sispopnet bootserv", start_response)
        else:
