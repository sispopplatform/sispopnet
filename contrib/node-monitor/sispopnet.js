...
const VERSION = 0.6
console.log('sispopnet launcher version', VERSION, 'registered')
...
function testDNSForSispopnet(server, cb) {
  const resolver = new dns.Resolver()
  resolver.setServers([server])
  // incase server is 127.0.0.1:undefined
  try {
    resolver.resolve('localhost.sispop', function (err, records) {
      if (err) console.error('NETWORK: localhost.sispop resolve err:', err)
      //log(server, 'localhost.sispop test results', records)
      cb(records)
    })
  } catch (e) {
    console.error('NETWORK: testDNSForSispopnet error, incorrect server?', server)
    cb()
  }
}
...
function lookup(host, cb) {
  ...
  //console.log('sispopnet lookup servers', runningConfig.dns.bind)
    ...
    if (err) {
      ...
      if (err.code == 'ENOTFOUND') {
        ...
        if (err.code == 'ETIMEOUT') {
          ...
          console.error('sispopnet lookup unknown err', err)
        }
    }
    //console.log(host, 'sispopnet dns test results', records)
    cb(records)
  })
}

function findSispopNetDNS(cb) {
  ...
  function checkDone() {
    if (shuttingDown) {
      //if (cb) cb()
      log('not going to start sispopnet, shutting down')
      return
    }
    checksLeft--
    if (checksLeft <= 0) {
  ...
  for(var i in resolvers) {
    ...
    if (idx != -1) {
      ...
      checksLeft++
      testDNSForSispopnet(server, function(isSispopnet) {
        if (isSispopnet) {
          // sispopnet
          console.log(server, 'is a sispopnet DNS server')
          servers.push(server)
        }
        checkDone()
      })
  ...
  */
  ...
  for (var i in localIPs) {
    ...
    checksLeft++
    testDNSForSispopnet(server, function (isSispopnet) {
      if (isSispopnet !== undefined) {
        // sispopnet
        log(server, 'is a sispopnet DNS server')
        servers.push(server)
      }
      checkDone()
    })
  ...
  for (var i in resolvers) {
    ...
    if (idx != -1) {
      ...
      testDNSForSispopnet(server, function (isSispopnet) {
        if (isSispopnet === undefined) {
          // not sispopnet
          log(server, 'is not a sispopnet DNS server')
          servers.push(server)
        }
        checkDone()
      })
      ...
      if (idx != -1) {
        ...
        resolver.resolve('localhost.sispop', function(err, records) {
          ...
          if (records === undefined) {
            // not sispopnet
            console.log(server, 'is not a sispopnet DNS server')
            servers.push(server)
          }
          checkDone()
        })
...
// this can really delay the start of sispopnet
...
function generateINI(config, need, markDone, cb) {
  ...
  //const data = fs.readFileSync(homeDir + '/.sispopnet/sispopnet.ini', 'utf-8')
  //const jConfig = iniToJSON(data)
  //console.dir(jConfig)
  //const iConfig = jsonToINI(jConfig)
  //console.log(iConfig)
  var upstreams, sispopnet_free53Ip, sispopnet_nic
  var use_sispopnet_rpc_port = config.rpc_port
  var sispopnet_bootstrap_path = homeDir + '/.sispopnet/bootstrap.signed'
  var sispopnet_nodedb = homeDir + '/.sispopnet/netdb'
  if (config.netid) {
    sispopnet_nodedb += '-' + config.netid
  }
  if (!fs.existsSync(sispopnet_nodedb)) {
    log('making', sispopnet_nodedb)
    mkDirByPathSync(sispopnet_nodedb)
  }
  var upstreamDNS_servers = []
  var params = {
    upstreamDNS_servers: upstreamDNS_servers,
    sispopnet_free53Ip: sispopnet_free53Ip,
    sispopnet_nodedb: sispopnet_nodedb,
    sispopnet_bootstrap_path: sispopnet_bootstrap_path,
    sispopnet_nic: sispopnet_nic,
    use_sispopnet_rpc_port: use_sispopnet_rpc_port,
  }
  if (config.bootstrap_url) {
    httpGet(config.bootstrap_url, function (bootstrapData) {
      if (bootstrapData) {
        cleanUpBootstrap = true
        const tmpRcPath = os.tmpdir() + '/' + randomString(8) + '.sispopnet_signed'
        fs.writeFileSync(tmpRcPath, bootstrapData, 'binary')
        log('boostrap wrote', bootstrapData.length, 'bytes to', tmpRcPath)
        //sispopnet_bootstrap_path = tmpRcPath
        params.sispopnet_bootstrap_path = tmpRcPath
    ...
    // seed version
    //params.sispopnet_bootstrap_path = ''
    markDone('bootstrap', params)
  }
  ...
  httpGet('http://' + config.rpc_ip + ':' + config.rpc_port, function (testData) {
    ...
    if (testData !== undefined) {
      ...
      use_sispopnet_rpc_port = use_sispopnet_rpc_port + 1
      params.use_sispopnet_rpc_port = use_sispopnet_rpc_port
    }
  ...
  getNetworkIP(function (e, ip) {
    ...
    sispopnet_nic = getIfNameFromIP(ip)
    params.sispopnet_nic = sispopnet_nic
    params.interfaceIP = ip
    log('detected outgoing interface', sispopnet_nic)
    ...
    findFreePort53(tryIps, 0, function (free53Ip) {
      if (free53Ip === undefined) {
        console.error('NETWORK: Cant automatically find an IP to put a sispopnet DNS server on')
        // can't handle the exits here because we don't know if it's an actual requirements
        if (done.dnsBind !== undefined) {
          process.exit()
        }
      }
      sispopnet_free53Ip = free53Ip
      params.sispopnet_free53Ip = free53Ip
...
function applyConfig(file_config, config_obj) {
  ...
  if (file_config.sispopd && file_config.sispopd.network == "test") {
...
function generateSerivceNodeINI(config, cb) {
  ...
  function markDone(completeProcess, params) {
    ...
    var keyPath = homeDir + '/.sispop/'
    //
    if (config.sispopd.data_dir) {
      keyPath = config.sispopd.data_dir
      // make sure it has a trailing slash
      if (keyPath[keyPath.length - 1] != '/') {
        keyPath += '/'
      }
    }
    if (config.sispopd.network == "test" || config.sispopd.network == "demo") {
      keyPath += 'testnet/'
    }
    keyPath += 'key'
    ...
    log('Drafting sispopnet service node config')
    ...
    runningConfig = {
      ...
      dns: {
        ...
        bind: params.sispopnet_free53Ip + ':53',
      },
      netdb: {
        dir: params.sispopnet_nodedb,
      },
      bind: {
        // will be set after
      },
      ...
      api: {
        enabled: true,
        bind: config.rpc_ip + ':' + params.use_sispopnet_rpc_port
      },
      sispopd: {
        enabled: true,
        jsonrpc: config.sispopd.rpc_ip + ':' + config.sispopd.rpc_port,
        username: config.sispopd.rpc_user,
        password: config.sispopd.rpc_pass,
    ...
    runningConfig.bind[params.sispopnet_nic] = config.public_port
    if (config.internal_port) {
      runningConfig.bind[params.sispopnet_nic] = config.internal_port
    }
...
function generateClientINI(config, cb) {
  ...
  function markDone(completeProcess, params) {
    ...
    if (!params.use_sispopnet_rpc_port) {
      // use default because we enable it
      params.use_sispopnet_rpc_port = 1190
    }
    log('Drafting sispopnet client config')
    runningConfig = {
      router: {
        nickname: 'ldl',
      },
...
var shuttingDown
var sispopnet
var sispopnetLogging = true

function preLaunchSispopnet(config, cb) {
  ...
  if (os.platform() == 'linux') {
    ...
    exec('getcap ' + config.binary_path, function (error, stdout, stderr) {
      ...
      // src/sispop-network/sispopnet = cap_net_bind_service,cap_net_admin+eip
  ...
  // sispopnet will crash if this file is zero bytes
  ...
  const tmpPath = tmpDir + '/' + randomString(8) + '.sispopnet_ini'
  cleanUpIni = true
  config.ini_writer(config, function (iniData) {
    if (shuttingDown) {
      //if (cb) cb()
      log('not going to write sispopnet config, shutting down')
      return
    }
...
function launchSispopnet(config, cb) {
  if (shuttingDown) {
    //if (cb) cb()
    log('not going to start sispopnet, shutting down')
    return
  ...
  if (!fs.existsSync(config.ini_file)) {
    log('sispopnet config file', config.ini_file, 'does not exist')
    process.exit()
  }
  ...
  sispopnet = spawn(config.binary_path, cli_options)

  if (!sispopnet) {
    console.error('failed to start sispopnet, exiting...')
    // proper shutdown?
    process.exit()
  }
  sispopnet.killed = false
  sispopnet.stdout.on('data', (data) => {
  ...
  sispopnet.stderr.on('data', (data) => {
    if (module.exports.onError) {
      module.exports.onError(data)
    }
  })

  sispopnet.on('close', (code) => {
    log(`sispopnet process exited with code ${code}`)
    // code 0 means clean shutdown
    sispopnet.killed = true
    // clean up
    ...
    if (!shuttingDown) {
      if (config.restart) {
        ...
        setTimeout(function () {
          log('sispop_daemon is still running, restarting sispopnet')
          launchSispopnet(config)
        }, 30 * 1000)
      } else {
        // don't restart...
      }
...
function checkConfig(config) {
  ...
  if (config.binary_path === undefined) config.binary_path = '/usr/local/bin/sispopnet'
...
function startServiceNode(config, cb) {
  ...
  // only us sispopnet devs will need to make our own seed node
  preLaunchSispopnet(config, function () {
    // test sispopd rpc port first
    // also this makes sure the service key file exists
    var url = 'http://' + config.sispopd.rpc_user + ':' + config.sispopd.rpc_pass + '@' + config.sispopd.rpc_ip + ':' + config.sispopd.rpc_port
    log('sispopnet waiting for sispopd RPC server')
    waitForUrl(url, function () {
      launchSispopnet(config, cb)
    })
  })
}
...
// intent to stop sispopnet and don't restart it
var retries = 0

function stop() {
  shuttingDown = true
  if (sispopnet && sispopnet.killed) {
    console.warn('sispopnet already stopped')
    retries++
    if (retries > 3) {
  ...
  log('requesting sispopnet be shutdown')
  if (sispopnet && !sispopnet.killed) {
    log('sending SIGINT to sispopnet', sispopnet.pid)
    process.kill(sispopnet.pid, 'SIGINT')
    sispopnet.killed = true
    // HACK: sispopnet on macos can not be killed if rpc port is in use
    var monitorTimerStart = Date.now()
    var monitorTimer = setInterval(function () {
      if (!isPidRunning(sispopnet.pid)) {
        // launcher can't exit until this interval is cleared
        clearInterval(monitorTimer)
      } else {
        var diff = Date.now() - monitorTimerStart
        if (diff > 15 * 1000) {
          // reach 15 secs and sispopnet is still running
          // escalate it
          console.error('Sispopnet is still running 15s after we intentionally stopped sispopnet?')
          process.kill(sispopnet.pid, 'SIGKILL')
        } else
        if (diff > 30 * 1000) {
          // reach 30 secs and sispopnet is still running
          // escalate it
          console.error('Sispopnet is still running 30s after we intentionally killed sispopnet?')
    ...
    setTimeout(function() {
      try {
        ...
        process.kill(sispopnet.pid, 0)
        log('sending SIGKILL to sispopnet')
        process.kill(sispopnet.pid, 'SIGKILL')
      } catch(e) {
        console.error('Launcher is still running 15s after we intentionally stopped sispopnet?')
        var handles = process._getActiveHandles()
        console.log('handles', handles.length)
        for(var i in handles) {
          var handle = handles[i]
...
function getPID() {
  return (sispopnet && !sispopnet.killed && sispopnet.pid) ? sispopnet.pid : 0
}

function enableLogging() {
  sispopnetLogging = true
}

function disableLogging() {
  console.log('Disabling sispopnet logging')
  sispopnetLogging = false
}

function getSispopNetIP(cb) {
  function checkDNS() {
    log('sispopnet seems to be running, querying', runningConfig.dns.bind)
    // where's our DNS server?
    //log('RunningConfig says our sispopnet\'s DNS is on', runningConfig.dns.bind)
    testDNSForSispopnet(runningConfig.dns.bind, function (ips) {
      //log('sispopnet test', ips)
      if (ips && ips.length) {
        cb(ips[0])
      } else {
        // , retrying
        console.error('cant communicate with sispopnet DNS')
        /*
        //process.exit()
        setTimeout(function() {
          getSispopNetIP(cb)
  ...
  if (runningConfig.api.enabled) {
    log('wait for sispopnet startup', runningConfig.api)
...
module.exports = {
  ...
  findSispopNetDNS: findSispopNetDNS,
  lookup: lookup,
  isRunning: isRunning,
  stop: stop,
  getSispopNetIP: getSispopNetIP,
  ...
  onMessage: function (data) {
    if (sispopnetLogging) {
      console.log(`sispopnet: ${data}`)
    }
  },
  onError: function (data) {
    console.log(`sispopneterr: ${data}`)
  },
}
