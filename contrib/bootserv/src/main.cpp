...
main(int argc, char* argv[])
{
  ...
  const char* confFile = sispopnet::bootserv::Config::DefaultPath;
  sispopnet::bootserv::Config config;

  sispopnet::bootserv::Handler_ptr handler;
  ...
  if(RunCron)
    handler = sispopnet::bootserv::NewCronHandler(std::cout);
  else
    handler = sispopnet::bootserv::NewCGIHandler(std::cout);
