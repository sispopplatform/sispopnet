...
namespace llarp
{
  ...
  FindFreeTun()
  {
    ...
    if(num == 255)
    {
      llarp::LogError("Could not find any free sispoptun interface names");
      return "";
    }
// include sispoptun prefix to communicate result is valid
#if defined(ANDROID) || defined(RPI)
    char buff[IFNAMSIZ + 1] = {0};
    snprintf(buff, sizeof(buff), "sispoptun%u", num);
    return buff;
#else
    return "sispoptun" + std::to_string(num);
#endif
  }
