...
namespace llarp
{
  ...
  Router::FromConfig(Config *conf)
  {
    ...
    // Sispopd Config
    usingSNSeed      = conf->sispopd.usingSNSeed;
    ident_keyfile    = conf->sispopd.ident_keyfile;
    whitelistRouters = conf->sispopd.whitelistRouters;
    sispopdRPCAddr     = conf->sispopd.sispopdRPCAddr;
    sispopdRPCUser     = conf->sispopd.sispopdRPCUser;
    sispopdRPCPassword = conf->sispopd.sispopdRPCPassword;
  ...
  Router::Run()
  {
    ...
    if(whitelistRouters)
    {
      ...
      rpcCaller->SetAuth(sispopdRPCUser, sispopdRPCPassword);
      while(!rpcCaller->Start(sispopdRPCAddr))
      {
        LogError("failed to start jsonrpc caller to ", sispopdRPCAddr);
#if defined(ANDROID) || defined(RPI)
        sleep(1);
#else
        std::this_thread::sleep_for(std::chrono::seconds(1));
      ...
      LogInfo("RPC Caller to ", sispopdRPCAddr, " started");
    }
